...
class V1ExampleServiceQueryRequest {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "V1ExampleServiceQueryRequest[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "V1ExampleServiceQueryRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<V1ExampleServiceQueryRequest> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <V1ExampleServiceQueryRequest>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = V1ExampleServiceQueryRequest.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, V1ExampleServiceQueryRequest> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = V1ExampleServiceQueryRequest.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<V1ExampleServiceQueryRequest>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<V1ExampleServiceQueryRequest>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = V1ExampleServiceQueryRequest.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
