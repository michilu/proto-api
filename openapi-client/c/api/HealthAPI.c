...
HealthAPI_healthCheck(apiClient_t *apiClient, char * service )
{
    ...
    v1_health_check_response_t *elementToReturn = v1_health_check_response_parseFromJSON(HealthAPIlocalVarJSON);
...
HealthAPI_healthWatch(apiClient_t *apiClient, char * service )
{
    ...
    // create the path
    long sizeOfPath = strlen("/healthWatch")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/healthWatch");
    ...
    //nonprimitive not container
    cJSON *HealthAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    stream_result_of_v1_health_check_response_t *elementToReturn = stream_result_of_v1_health_check_response_parseFromJSON(HealthAPIlocalVarJSON);
    cJSON_Delete(HealthAPIlocalVarJSON);
    if(elementToReturn == NULL) {
        // return 0;
    }
