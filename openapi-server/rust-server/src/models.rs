...
impl std::fmt::Display for CheckResponseServingStatus {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match *self {
            CheckResponseServingStatus::UnknownUnspecified => write!(f, "SERVING_STATUS_UNKNOWN_UNSPECIFIED"),
            CheckResponseServingStatus::Serving => write!(f, "SERVING_STATUS_SERVING"),
            CheckResponseServingStatus::NotServing => write!(f, "SERVING_STATUS_NOT_SERVING"),
        }
    }
}

impl std::str::FromStr for CheckResponseServingStatus {
    type Err = String;

    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        match s {
            "SERVING_STATUS_UNKNOWN_UNSPECIFIED" => std::result::Result::Ok(CheckResponseServingStatus::UnknownUnspecified),
            "SERVING_STATUS_SERVING" => std::result::Result::Ok(CheckResponseServingStatus::Serving),
            "SERVING_STATUS_NOT_SERVING" => std::result::Result::Ok(CheckResponseServingStatus::NotServing),
...
pub struct V1CheckResponse {
    #[serde(rename = "status")]
    pub status: models::CheckResponseServingStatus,
...
impl V1CheckResponse {
    #[allow(clippy::new_without_default)]
    pub fn new(status: models::CheckResponseServingStatus, ) -> V1CheckResponse {
        V1CheckResponse {
            status,
        }
    }
...
impl std::str::FromStr for V1CheckResponse {
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        #[allow(dead_code)]
        struct IntermediateRep {
            pub status: Vec<models::CheckResponseServingStatus>,
        }
        ...
        while key_result.is_some() {
            ...
            if let Some(key) = key_result {
                ...
                match key {
                    #[allow(clippy::redundant_clone)]
                    "status" => intermediate_rep.status.push(<models::CheckResponseServingStatus as std::str::FromStr>::from_str(val).map_err(|x| x.to_string())?),
