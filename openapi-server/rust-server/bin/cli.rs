...
#[structopt(
    name = "An example of generating swagger via gRPC ecosystem.",
    version = "1.0",
    about = "CLI access to An example of generating swagger via gRPC ecosystem."
)]
struct Cli {
...
async fn main() -> Result<()> {
    ...
    let result = match args.operation {
        Operation::ExampleServiceQuery {
            id,
            body,
        } => {
            info!("Performing a ExampleServiceQuery request on {:?}", (
                &id
            ));

            let result = client.example_service_query(
                id,
                body,
            ).await?;
            debug!("Result: {:?}", result);
